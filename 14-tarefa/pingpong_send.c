...
#define MAX_SIZE (1024 * 1024) // 1MB, ajuste se quiser mensagens até maiores
...
int main(int argc, char **argv)
{
	...
	// Testa mensagens variando de 8 bytes até MAX_SIZE (em potências de 2)
	for (int msg_size = 8; msg_size <= MAX_SIZE; msg_size *= 2)
	{
		...
		if (rank == 0)
		{
			// Tempo de uma PERNA da viagem de ida e volta (RTT / 2)
